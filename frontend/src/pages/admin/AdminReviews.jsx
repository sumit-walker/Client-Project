...
import { Star, Trash2, Pencil, Check, X, ThumbsUp, EyeOff } from 'lucide-react'
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {reviews.map((r, i) => (
            ...
            >
                  ...
                  {editing === r._id ? (
                    <div className="flex gap-2">
                      ...
                      <div className="flex flex-col gap-1">
                        <button type="button" onClick={() => saveEditMutation.mutate({ id: r._id, data: { text: editText } })} className="btn btn-primary btn-xs rounded-xl text-white"><Check className="size-3" /></button>
                        <button type="button" onClick={() => setEditing(null)} className="btn btn-ghost btn-xs rounded-xl"><X className="size-3" /></button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mt-4 pt-3 border-t border-base-200">
                <div className="flex items-center gap-1">
                  {isApproved(r) ? (
                    <button
                      type="button"
                      onClick={() => toggleApprovalMutation.mutate({ id: r._id, isApproved: false })}
                      className="btn btn-ghost btn-xs rounded-lg gap-1.5 px-2.5 text-warning hover:text-warning"
                    ...
                    <button
                      ...
                      className="btn btn-ghost btn-xs rounded-lg gap-1.5 px-2.5 text-success hover:text-success"
                      disabled={toggleApprovalMutation.isPending}
                    >
                ...
                <div className="flex items-center gap-0.5">
                  <button type="button" onClick={() => startEdit(r)} className="btn btn-ghost btn-xs rounded-lg gap-1.5 px-2.5 text-base-content/50 hover:text-base-content">
                    <Pencil className="size-3.5" /> Edit
                  </button>
                  <button type="button" onClick={() => setDeleteTarget(r)} className="btn btn-ghost btn-xs rounded-lg gap-1.5 px-2.5 text-base-content/50 hover:text-error">
